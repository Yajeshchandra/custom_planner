...
bool RRTStar::pathPlanning(std::list<std::pair<float, float>> &path) {
  goal_reached_ = false;
  ROS_INFO("Start RRT* path planning");
  ROS_INFO("Cost Map Size: %d x %d", costmap_->getSizeInCellsX(), costmap_->getSizeInCellsY());
  ROS_INFO("Wx,Wy Origin: %f, %f ,%d",costmap_->getOriginX(), costmap_->getOriginY(), cd_.isThisPointCollides(costmap_->getOriginX(), costmap_->getOriginY()));
  int mx,my;
  cd_.worldToMap(costmap_->getOriginX(), costmap_->getOriginY(), mx, my);;
  ROS_INFO("Mx,My Origin: %d, %d",mx, my);
  if (cd_.isThisPointCollides(goal_point_.first, goal_point_.second)) {
    ROS_INFO("W-Point: (%f, %f)", goal_point_.first, goal_point_.second);
    ROS_ERROR("Goal point chosen is NOT in the FREE SPACE! Choose other goal!");
    return false;
  }
  ROS_INFO("Path planning goal point is in the FREE SPACE!");
  // Start Node
